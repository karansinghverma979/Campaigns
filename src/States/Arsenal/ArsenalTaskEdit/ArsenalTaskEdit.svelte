<script>
  ...
  import { Plus, X, Tag as TagIcon, Target, ShieldAlert, Sparkles, Check, Flame, AlertTriangle, Eye, Loader2, AlertCircle, Save } from 'lucide-svelte';
  ...
  $effect(() => {
    if (task) {
      ...
      tags = task.tags ? task.tags.map(t => t.tag_name.toUpperCase()) : [];
    }
  });
  ...
  <div class="modal-card" onclick={handleModalClick}>
    ...
    <div class="modal-header">
      <div class="header-title-block">
        ...
        <div>
          <h2 class="modal-title-text">EDIT CAMPAIGN INTEL MANIFEST</h2>
          <p class="modal-subtitle">Modify parameters for active Arsenal campaign task #{task.id}</p>
        </div>
      </div>
    ...
    <form onsubmit={handleSave} class="form-body">
      
      <!-- Field 1: Campaign Title -->
      <div class="field-group">
        <div class="label-row">
          <label for="edit-campaign-title">CAMPAIGN TITLE <span class="req-star">*</span></label>
      ...
      <div class="field-group">
        <div class="label-row">
          <label for="edit-tag-search-input">TAGS & CLASSIFICATION ASSIGNMENTS</label>
            ...
            <div class="tag-suggestions-popover">
              ...
              {:else if filteredTagSuggestions.length === 0 && (!tagInput.trim() || exactMatchExists)}
                <div class="empty-tag-popover">
                  Type tag name & press Enter to attach classification tag.
                </div>
              {/if}
            </div>
          {/if}
