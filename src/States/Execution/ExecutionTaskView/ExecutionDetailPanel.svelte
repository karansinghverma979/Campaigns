<script>
  ...
  import { 
    ...
    X, ChevronUp, Calendar, AlertTriangle, RefreshCw, ListTodo
  } from 'lucide-svelte';
  ...
  async function addSubtask() {
    ...
    if (!titleTrimmed) {
      store.showToast('Enter a tactical subtask title first.', 'warning');
      return;
    }
    if (!currentTask || !currentTask.id) return;
  ...
  function moveSubtaskUp(index) {
    if (index <= 0) {
      store.showToast('Tactical Block: Subtask is already at top position.', 'warning');
      return;
    }
  ...
  function handleBackdropClick(e) {
    if (canClose && e && e.target === e.currentTarget) {
      onClose();
    }
  }
</script>
...
<div class="panel right">
  ...
  <div class="panel-body">
    ...
    <div class="date-badges-row">
      <span class="date-badge"><Calendar size={12} /> Logged: {taskOriginDate}</span>
      <span class="date-badge deadline"><Clock size={12} /> Target Deadline: {taskDeadline}</span>
      <span class="date-badge permit">Permits: [{taskRescheduleCount}/2]</span>
    </div>
      ...
      <div class="tags-section">
        ...
        <div class="tags-row">
          {#each taskTags as tag}
            <!-- svelte-ignore a11y_click_events_have_key_events -->
            <!-- svelte-ignore a11y_no_static_element_interactions -->
            <span class="tag" onclick={() => store.showToast(`Tag Filter: #${tag.tag_name}`, 'info')} title="Click to focus tag #{tag.tag_name}">
    ...
    <div class="deploy-modal-dialog" onclick={(e) => e.stopPropagation()}>
      <div class="deploy-dialog-header">
        ...
        <div>
          ...
          <p class="deploy-dialog-sub">Permit Count: [{taskRescheduleCount}/2 Used]</p>
        </div>
